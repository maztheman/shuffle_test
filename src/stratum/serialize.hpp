...
#ifdef WIN32
#include <stratum/compat/endian.hpp>
#else
#include <endian.h>
#endif
...
#define READWRITE(obj)      (stratum::SerReadWrite(s, (obj), nType, nVersion, ser_action))
...
template<typename Stream>
inline void WriteCompactSize(Stream& os, uint64_t nSize)
{
    if (nSize < 253)
    {
...
template<typename Stream>
inline uint64_t ReadCompactSize(Stream& is)
{
...
inline void WriteVarInt(Stream& os, I n)
{
 ...
 */
extern inline unsigned int GetSerializeSize(const script::CScript& v, int nType, int nVersion);
template<typename Stream> void Serialize(Stream& os, const script::CScript& v, int nType, int nVersion);
template<typename Stream> void Unserialize(Stream& is, script::CScript& v, int nType, int nVersion);
...
void Serialize_impl(Stream& os, const std::vector<T, A>& v, int nType, int nVersion, const V&)
{
    WriteCompactSize(os, v.size());
    for (typename std::vector<T, A>::const_iterator vi = v.begin(); vi != v.end(); ++vi)
        stratum::Serialize(os, (*vi), nType, nVersion);
}
 ...
 * others derived from vector
 */
inline unsigned int GetSerializeSize(const script::CScript& v, int nType, int nVersion)
{
    return GetSerializeSize((const std::vector<unsigned char>&)v, nType, nVersion);
}

template<typename Stream>
void Serialize(Stream& os, const script::CScript& v, int nType, int nVersion)
{
    Serialize(os, (const std::vector<unsigned char>&)v, nType, nVersion);
}

template<typename Stream>
void Unserialize(Stream& is, script::CScript& v, int nType, int nVersion)
{
...
template<typename Stream, typename T>
inline void Serialize(Stream& os, const boost::optional<T>& item, int nType, int nVersion)
{
...
template<typename Stream, typename T>
inline void Unserialize(Stream& is, boost::optional<T>& item, int nType, int nVersion)
{
    unsigned char discriminant = 0x00;
...
inline unsigned int GetSerializeSize(const boost::array<T, N> &item, int nType, int nVersion)
{
    unsigned int size = 0;
...
inline void Serialize(Stream& os, const boost::array<T, N>& item, int nType, int nVersion)
{
    for (size_t i = 0; i < N; i++) {
...
template<typename Stream, typename T, std::size_t N>
inline void Unserialize(Stream& is, boost::array<T, N>& item, int nType, int nVersion)
{
    for (size_t i = 0; i < N; i++) {
...
inline unsigned int GetSerializeSize(const std::pair<K, T>& item, int nType, int nVersion)
{
    return GetSerializeSize(item.first, nType, nVersion) + GetSerializeSize(item.second, nType, nVersion);
}

template<typename Stream, typename K, typename T>
inline void Serialize(Stream& os, const std::pair<K, T>& item, int nType, int nVersion)
{
...
inline unsigned int GetSerializeSize(const std::map<K, T, Pred, A>& m, int nType, int nVersion)
{
...
inline void Serialize(Stream& os, const std::map<K, T, Pred, A>& m, int nType, int nVersion)
{
...
template<typename Stream, typename K, typename T, typename Pred, typename A>
inline void Unserialize(Stream& is, std::map<K, T, Pred, A>& m, int nType, int nVersion)
{
    m.clear();
    unsigned int nSize = ReadCompactSize(is);
...
inline unsigned int GetSerializeSize(const std::set<K, Pred, A>& m, int nType, int nVersion)
{
    unsigned int nSize = GetSizeOfCompactSize(m.size());
...
inline void Serialize(Stream& os, const std::set<K, Pred, A>& m, int nType, int nVersion)
{
    WriteCompactSize(os, m.size());
    for (typename std::set<K, Pred, A>::const_iterator it = m.begin(); it != m.end(); ++it)
...
template<typename Stream, typename K, typename Pred, typename A>
inline void Unserialize(Stream& is, std::set<K, Pred, A>& m, int nType, int nVersion)
{
    m.clear();
    unsigned int nSize = ReadCompactSize(is);
 ...
 */
template<typename T, typename A>
inline unsigned int GetSerializeSize(const std::list<T, A>& l, int nType, int nVersion)
{
...
template<typename Stream, typename T, typename A>
inline void Serialize(Stream& os, const std::list<T, A>& l, int nType, int nVersion)
{
...
template<typename Stream, typename T, typename A>
inline void Unserialize(Stream& is, std::list<T, A>& l, int nType, int nVersion)
{
    l.clear();
    unsigned int nSize = ReadCompactSize(is);
...
public:
    ...
    {
        stratum::Serialize(*this, obj, nType, nVersion);
        return (*this);
    }
